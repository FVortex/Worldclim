...
P.tragium_first_65points_coords <- unname(as.matrix(P.tragium_coords[1:65,]))

P.tragium_first_65points <- unname(as.matrix(P.tragium_variables[1:65,]))
P.tragium_rest_66points <- as.matrix(P.tragium_variables[-c(1:65),])

P.tragium_first_65points_coords <- unname(as.matrix(P.tragium_coords[1:65,]))
P.tragium_rest_66points_coords <- as.matrix(P.tragium_coords[-c(1:65),])


colnames(background_130points_variables) <- colnames(P.tragium_first_65points)

p_train <- c(rep(1, nrow(background_130points_variables)), rep(0, nrow(P.tragium_first_65points)))
p_test <- c(rep(1, nrow(background_rest_120points_coords)), rep(0, nrow(P.tragium_rest_66points)))

training <- ((rbind(background_130points_variables, P.tragium_first_65points)))
testing <- as.matrix(unname(rbind(as.matrix(background_rest_120points_variables), P.tragium_rest_66points)))
testing_coords <-  as.matrix(unname(rbind(as.matrix(background_rest_120points_coords), P.tragium_rest_66points_coords)))


colnames(training) <- colnames(P.tragium_first_65points)
...
df_testing <- as.data.frame(scale(training[complete.cases(testing),], center = T, scale = T))
...
for (i in methods_to_loop){
  ...
  dummy <- matrix(NA, nrow(P.tragium_rest_66points), ncol = 2)
  tmp3  <- as.data.frame(unname(rbind(background_rest_120points_coords, dummy))[which(tmp2 ==1),])
